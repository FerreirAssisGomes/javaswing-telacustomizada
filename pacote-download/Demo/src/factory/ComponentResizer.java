...
Postado por Rob Camick em 13 de setembro de 2009

Uma entrada recente em Moving Windows discutiu como você pode adicionar funcionalidades para mover um componente ou uma janela não decorada. Hoje veremos como adicionar funcionalidades de redimensionamento a esses mesmos componentes.

A classe ComponentResizer será usada para implementar a funcionalidade de redimensionamento. Seu design é semelhante à classe ComponentMover , pois você precisa especificar as inserções de arrasto para o componente. As inserções são usadas para reconhecer quando o mouse é movido sobre uma borda redimensionável. Quando isso acontece, o cursor é alterado para o cursor apropriado para indicar a borda de redimensionamento. Uma diferença é que um valor inserido como 0 para qualquer borda implica que a borda não pode ser redimensionada. O valor padrão é 5 para cada borda. As mesmas inserções de arrastar devem ser usadas para o ComponentResizer e o ComponentMover, para que as classes não fiquem confusas quanto à necessidade de redimensionar ou mover um componente.

Os outros recursos da classe são os seguintes:

tamanho do snap (padrão 1) - especifica o incremento de redimensionamento. Essa é a distância que o mouse deve ser arrastado antes que a borda mude de tamanho. A borda será ajustada para esse valor assim que a arrastar atingir a marca do meio.
tamanho mínimo - especifique o tamanho mínimo do componente. O tamanho mínimo é limitado pelas inserções de arrasto. (ou seja, a largura não pode ser menor que a soma dos valores à esquerda e à direita)
tamanho máximo - especifique o tamanho máximo do componente. O tamanho do componente também será limitado pelo tamanho de seu pai. Ao redimensionar uma janela ou moldura, a restrição é o tamanho da área de trabalho. Para outros componentes, a restrição é o contêiner pai.
O uso do ComponentResizer é direto. Existem vários construtores que você pode usar. Um exemplo seria:

ComponentResizer cr = new ComponentResizer();
cr.setSnapSize(new Dimension(10, 10));
cr.registerComponent(component1, component2);

Esperamos que esta classe forneça a flexibilidade necessária ao redimensionar componentes.
 */
/**
